...
Fb_bottom=Fb(faceBoundaryMarker==5,:);
indBottom=unique(Fb_bottom(:));

indBoundary=unique(Fb(:));

logicBottom=false(size(V,1),1);
logicBottom(indBottom)=1; 
logicBarLeftRigth=(V(:,1)<=barDiameter/2) & (V(:,1)>=-barDiameter/2);

logicBarBottom=logicBarLeftRigth & logicBottom; 
indBarBottom=find(logicBarBottom);

Vs=V(logicBarBottom,:);

plotV(Vs,'r.','MarkerSize',25);
X=Vs(:,1);
Y=Vs(:,2);
Z=Vs(:,3);

%Create angular coordinates for set
t=Vs(:,1); %Initiate as X
t=t-min(t(:)); %0-...
t=(1-(t./max(t(:)))); %0-1
t=t.*pi; %0-pi

Xn=barDiameter/2*cos(t);
Yn=Vs(:,2);
Zn=barDiameter/2*sin(t);

Vn=[Xn(:) Yn(:) Zn(:)];

plotV(Vn,'b.','MarkerSize',25);
...
% hp=patch('Faces',F,'Vertices',V,'FaceColor','r','FaceAlpha',0.1,'lineWidth',edgeWidth,'edgeColor',edgeColor);

hpp1=plotV(Vs,'r.','MarkerSize',25);
hpp2=plotV(Vn,'b.','MarkerSize',25);
...
for q=1:1:ns
    ...
    %Change coordinates
    Vss(indBarBottom,1)=XX(:,q);
    Vss(indBarBottom,2)=YY(:,q);
    Vss(indBarBottom,3)=ZZ(:,q);
    
    %Smoothen
    for qs=1:1:nSmooth        
        [Vss]=tesSmooth(E,Vss,[],smoothPar);        
        for qc=1:1:6
            Fb_now=Fb(faceBoundaryMarker==qc,:);
            indNow=unique(Fb_now(:));
            indNow=indNow(~ismember(indNow,indBarBottom));
            switch qc
                case 1
                    Vss(indNow,1)=V(indNow,1);
                case 2
                    Vss(indNow,1)=V(indNow,1);
                case 3
                    Vss(indNow,2)=V(indNow,2);
                case 4
                    Vss(indNow,2)=V(indNow,2);
                case 5
                    Vss(indNow,3)=V(indNow,3);                    
                case 6
                    Vss(indNow,3)=V(indNow,3);
            end
        end        
...
delete(hpNow);
hpNow=patch('Faces',F,'Vertices',Vss,'FaceColor','b','FaceAlpha',1,'lineWidth',edgeWidth,'edgeColor',edgeColor);
