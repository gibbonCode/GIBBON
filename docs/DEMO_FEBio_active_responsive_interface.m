...
v_fib1=[0 0 1];
V_fib1=v_fib1(ones(size(E1,1),1),:);
X=V(:,1);
Y=V(:,2);
Z=V(:,3);
VE1=[mean(X(E1),2) mean(Y(E1),2) mean(Z(E1),2)];

VE2=[mean(X(E2),2) mean(Y(E2),2) mean(Z(E2),2)];
VE2n=VE2;
VE2n(:,3)=0;
V_fib2=vecnormalize(cross(VE2n,v_fib1(ones(size(VE2,1),1),:)));

V_fib=[V_fib1;V_fib2];

[a_fib,d_fib]=vectorOrthogonalPair(V_fib);
...
gpatch(Fb,V,Cb,'none',0.5);
% quiverVec(VE1,V_fib1,1,'k','none');
% quiverVec(VE2,V_fib2,2,'r','none');
quiverVec(VE,a_fib,2,'r','none');
quiverVec(VE,d_fib,1,'g','none');
...
VF_E(:,:,1)=a_fib; %a1 ~ e1 ~ X or first direction
VF_E(:,:,2)=d_fib; %a2 ~ e2 ~ Y or second direction
...
FEB_struct.Materials{1}.Solid{2}.Values={T0_1,0,0};
...
% FEB_struct.Materials{2}.AnisoType='mat_axis';
FEB_struct.Materials{2}.Solid{1}.Type='Ogden unconstrained';
FEB_struct.Materials{2}.Solid{1}.Properties={'c1','m1','c1','m1','cp'};
FEB_struct.Materials{2}.Solid{1}.Values={c2,2,c2,-2,k};

FEB_struct.Materials{2}.Solid{2}.Type='prescribed uniaxial active contraction';
FEB_struct.Materials{2}.Solid{2}.Properties={'T0','theta','phi'};
...
FEB_struct.LoadData.LoadCurves.loadPoints={[0 0;1 1;2 1;3 0.5;]; [0 0;1 0;2 1;3 1;]};
