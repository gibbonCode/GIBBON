...
a_view = get(h_ax,'View');
a = linspace(a_view(1),a_view(1)+360,nSteps);
...
for q=1:1:nSteps
    ...
    animStruct.Set{q}={[aNow a_view(2)]}; %Property values for to set in order to animate
end
...
faceAlphaLevels=linspace(1,0.01,nSteps);
...
if is_octave
	% Does not work yet in Octave
	% loadName=fullfile(loadPath,'exampleSavedAnim8Figure.ofig');
	% anim8(loadName);
else
