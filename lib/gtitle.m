...
unitsNow=get(hf,'Units');
set(hf,'Units','Points');

figPosition=get(hf,'Position');
textBoxWidth=figPosition(3);
extentNow = get(hTextInfo,'Extent');

textBoxHeight=extentNow(4).*ceil(extentNow(3)./figPosition(3));
textPosition=[0 figPosition(4)-textBoxHeight textBoxWidth textBoxHeight];
set(hTextInfo,'Position',textPosition);

set(hf,'Units',unitsNow);
end
