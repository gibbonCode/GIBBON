function [varargout]=amfStruct2xml(varargin)

% function [domNode]=amfStruct2xml(parseStruct,fileName,optionStruct)
% ------------------------------------------------------------------------
%
% This function provides the basis for coding AMF XML input file content
...
domNode = com.mathworks.xml.XMLUtils.createDocument('amf'); %Create the overall amf field

%% Add comment
% commentString = ['Created using GIBBON, ',char(datetime('now','TimeZone','local','Format','yyyy-dd-MM HH:mm:ss'))];
...
%% Add content to XML
[domNode]=struct2xmlStep(domNode,rootNode,parseStruct,optionStruct,fileName);
...
function [domNode,rootNode]=struct2xmlStep(domNode,rootNode,parseStruct,optionStruct,fileName)
...
if isempty(parseStruct) %throw error
    error('Empty structure component encountered. Note this may occur when index is skipped in amf structure')
end
...
elseif isstruct(currentFieldValue)
    ...
    [domNode,elementNode]=struct2xmlStep(domNode,elementNode,currentFieldValue,optionStruct,fileName); %Add whatever is nested in here
elseif iscell(currentFieldValue)
    for q_cell=1:1:numel(currentFieldValue)
        ...
        [domNode,elementNode]=struct2xmlStep(domNode,elementNode,parseStructSub,optionStruct,fileName); %Add whatever is nested in here
    end
end
