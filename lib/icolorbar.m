...
if isempty(h)
   ...
   figure(hf);
   CurrentAxes = get(hf,'CurrentAxes');
   if isempty(CurrentAxes) %isempty(get(hf,'CurrentAxes'))
       h=axes;
   else
       h=gca;
   end
...
hc=colorbar;
tickSet = cLim(1):1:cLim(2);
try
  set(hc,'Ticks',tickSet);
catch

end

cn=resampleColormap(c,numel(tickSet));
