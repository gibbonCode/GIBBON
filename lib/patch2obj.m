...
%If coordinates are 2D add zero Z-coordinates
if size(V,2)==2
    V(:,3)=0;
end
...
FF=zeros(size(F,1),size(F,2)+2*size(F,2));
...
%Create text
t_form_p=repmat([formatInteger,'/'],1,3);
t_form_p=t_form_p(1:end-1); %Crop trailing delimiter
